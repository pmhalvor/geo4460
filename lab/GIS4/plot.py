...
def get_columns_of_interest(df: pd.DataFrame) -> list:
    return [
        col for col in df.columns if col.startswith("THAW") or col.startswith("SNOW")
    ]
...
def plot_snow_melt_regression_lines(data: pd.DataFrame):
    ...
    colors = plt.cm.tab10_r(
        np.linspace(
            0, 1, len([col for col in columns_of_interest if col.startswith("THAW")])
        )
    )
    ...
    for i, thaw_column in enumerate(
        [col for col in columns_of_interest if col.startswith("THAW")]
    ):
        ...
        # Scatter plot
        ax_scatter.scatter(
            snow200705, data[thaw_column], label=f"{thaw_column}", color=colors[i]
        )
        ...
        ax_regression.plot(
            x,
            np.polyval(poly1, x),
            label=f"...{thaw_column[-2:]} (1st order)",
            linestyle="-",
            color=colors[i],
            linewidth=2,
        )
        ax_regression.plot(
            x,
            np.polyval(poly2, x),
            label=f"...{thaw_column[-2:]} (2nd order)",
            linestyle="--",
            color=colors[i],
            linewidth=2,
        )
    ...
    ax_scatter.set_title(
        "Scatter Plot of SNOW200705 vs THAW* Columns", fontsize="xx-large"
    )
    ax_scatter.set_xlabel("SNOW200705")
    ax_scatter.set_ylabel("THAW Values")
    ax_scatter.legend(
        title="Legend", bbox_to_anchor=(1.05, 1), loc="upper left", fontsize="xx-large"
    )
    ax_scatter.grid(alpha=0.5)

    # Customize regression plot axis
    ax_regression.set_title(
        "Regression Lines for SNOW200705 vs THAW* Columns", fontsize="xx-large"
    )
