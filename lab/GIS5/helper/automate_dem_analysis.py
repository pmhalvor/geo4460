...
def get_common_extent_and_crs(layers):
    ...
    if crs is None:
        print(
            "Warning: Input layer CRS is None. Assuming a projected CRS for calculations."
        )
...
if __name__ == "__main__":
    ...
    contour_points_with_value_path = os.path.join(
        OUTPUT_DIR, "contour_points_with_value.shp"
    )

    try:
        # Convert contour lines to raster, burning HOEYDE attribute
        print(
            f"    - Converting contour lines to raster ({os.path.basename(contour_raster_path)})..."
        )
        ...
        print(
            f"    - Converting contour raster to points ({os.path.basename(contour_points_with_value_path)})..."
        )
        wbt.raster_to_vector_points(
            i=contour_raster_path, output=contour_points_with_value_path
        )
        ...
        if not os.path.exists(contour_points_with_value_path):
            raise FileNotFoundError(
                f"Points file with values not created: {contour_points_with_value_path}"
            )
        ...
        if not os.path.exists(dem_interp_path):
            raise FileNotFoundError(
                f"Interpolated DEM file not created: {dem_interp_path}"
            )
    ...
    try:
        ...
        if not os.path.exists(contour_points_with_value_path):
            raise FileNotFoundError(
                f"Contour points file needed for TIN Gridding not found: {contour_points_with_value_path}"
            )
        ...
        if not os.path.exists(dem_topo_path):
            raise FileNotFoundError(
                f"TIN Gridding DEM file not created: {dem_topo_path}"
            )
    ...
    if not os.path.exists(dem_interp_path):
        ...
        try:
            ...
            print(
                f"  - Copying original points shapefile to {points_extracted_path}..."
            )
            ...
            if len(new_col_1) == 1:
                ...
                points_extracted_gdf_after_1.rename(
                    columns={added_col_name_1: "DEM_Interp"}, inplace=True
                )
                print(
                    f"    - Renamed extracted value column '{added_col_name_1}' to 'DEM_Interp'"
                )
                # Save the renamed file before the next extraction
                points_extracted_gdf_after_1.to_file(
                    points_extracted_path, driver="ESRI Shapefile"
                )
            else:
                print(
                    f"    - Warning: Could not uniquely identify column added by first extraction. Found: {new_col_1}"
                )
                ...
                if "VALUE1" in cols_after_1:
                    ...
                    points_extracted_gdf_after_1.rename(
                        columns={"VALUE1": "DEM_Interp"}, inplace=True
                    )
                    points_extracted_gdf_after_1.to_file(
                        points_extracted_path, driver="ESRI Shapefile"
                    )
            ...
            points_extracted_gdf_before_2 = gpd.read_file(
                points_extracted_path
            ...
            if len(new_col_2) == 1:
                ...
                points_extracted_gdf_after_2.rename(
                    columns={added_col_name_2: "DEM_Topo"}, inplace=True
                )
                print(
                    f"    - Renamed second extracted value column '{added_col_name_2}' to 'DEM_Topo'"
                )
            else:
                print(
                    f"    - Warning: Could not uniquely identify column added by second extraction. Found: {new_col_2}"
                )
                ...
                if "VALUE1" in new_col_2:  # Check if 'VALUE1' is the *new* column
                    ...
                    points_extracted_gdf_after_2.rename(
                        columns={"VALUE1": "DEM_Topo"}, inplace=True
                    )
                elif (
                    "VALUE1" in cols_after_2 and "DEM_Topo" not in cols_after_2
            ...
            if all(col in points_extracted_gdf.columns for col in required_cols):
                ...
                if n_points > 0:
                    ...
                    rmse_data = {
                        "DEM_Type": [
                            "Interpolated (Natural Neighbour)",
                            "TIN Gridding",
                        ],
                    ...
                    print(
                        "    - Error: No valid points found after dropping NaN/Inf values. Cannot calculate RMSE."
                    )

            else:
                print(
                    f"    - Error: Missing one or more required columns for RMSE calculation: {required_cols}"
                )
                print(
                    f"    - Available columns: {points_extracted_gdf.columns.tolist()}"
                )
    ...
    if not os.path.exists(dem_interp_path) or not os.path.exists(dem_topo_path):
        ...
        try:
            ...
            print(
                f"    - Contours from Interpolated DEM saved to: {contours_interp_path}"
            )
            wbt.contours_from_raster(
                i=dem_topo_path, output=contours_topo_path, interval=CONTOUR_INTERVAL
            )
            ...
            print(
                f"    - Hillshade from Interpolated DEM saved to: {hillshade_interp_path}"
            )
            wbt.hillshade(dem=dem_topo_path, output=hillshade_topo_path)
            ...
            wbt.subtract(
                input1=dem_topo_path, input2=dem_interp_path, output=dem_diff_path
            )
            ...
            wbt.slope(dem=dem_topo_path, output=slope_topo_path)
