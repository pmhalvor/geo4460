...
def assess_dem_quality(
    ...
    points_extracted_path = output_files.get_full_path(
        "points_extracted_shp", output_dir
    )
    ...
    if not dem_interp_path.exists():
        print(
            f"Error: Interpolated DEM not found ({dem_interp_path}). Skipping RMSE calculation."
        )
        return  # Or raise error if this is critical
    if not dem_topo_path.exists():
        print(
            f"Error: Topo DEM not found ({dem_topo_path}). Skipping RMSE calculation."
        )
        # Continue to check other DEMs
    if not dem_toporaster_all_path.exists():
        print(
            f"Error: TopoRaster_all DEM not found ({dem_toporaster_all_path}). Skipping RMSE calculation for this DEM."
        )
        # Continue to check other DEMs
    if not points_shp_path.exists():
        print(
            f"Error: Points shapefile not found ({points_shp_path}). Skipping RMSE calculation."
        )
        return  # Cannot proceed without points
    if point_elev_field is None:
        print(
            f"Error: Unknown elevation field in points layer. Skipping RMSE calculation."
        )
        return
    ...
    try:
        # --- Prepare Points File ---
        print(
            f"  - Copying original points shapefile to {points_extracted_path} for modification..."
        )
        ...
        print(
            f"  - Extracting values from Interpolated DEM ({output_files.dem_interpolated_tif})..."
        )
        ...
        if len(new_cols_interp) == 1:
            ...
            points_gdf_after_interp.rename(
                columns={added_col_interp: interp_col_name}, inplace=True
            )
            print(
                f"    - Renamed extracted value column '{added_col_interp}' to '{interp_col_name}'"
            )
        elif "VALUE1" in cols_after_interp and interp_col_name not in cols_after_interp:
            print(
                f"    - Warning: Could not uniquely identify new column. Attempting to rename 'VALUE1' to '{interp_col_name}'."
            )
            points_gdf_after_interp.rename(
                columns={"VALUE1": interp_col_name}, inplace=True
            )
        elif interp_col_name not in cols_after_interp:
            print(
                f"    - Warning: Could not identify or rename column for Interpolated DEM. Found new: {new_cols_interp}. All: {list(cols_after_interp)}"
            )
        ...
        if interp_col_name:  # Only save if rename was successful or attempted
            points_gdf_after_interp.to_file(
                str(points_extracted_path), driver="ESRI Shapefile"
            )
        cols_before_topo_extraction = set(
            points_gdf_after_interp.columns
        )  # Update baseline columns

        # --- Extract Values from Topo DEM ---
        print(
            f"  - Extracting values from TIN Gridding DEM ({output_files.dem_topo_tif})..."
        )
        wbt.extract_raster_values_at_points(
        ...
        if len(new_cols_topo) == 1:
            ...
            points_gdf_after_topo.rename(
                columns={added_col_topo: topo_col_name}, inplace=True
            )
            print(
                f"    - Renamed second extracted value column '{added_col_topo}' to '{topo_col_name}'"
            )
        elif (
            "VALUE1" in new_cols_topo
        ):  # Check if 'VALUE1' is the *new* column this time
            print(
                f"    - Warning: Could not uniquely identify new column. Attempting to rename 'VALUE1' to '{topo_col_name}'."
            )
            points_gdf_after_topo.rename(
                columns={"VALUE1": topo_col_name}, inplace=True
            )
        elif (
            "VALUE1" in cols_after_topo and topo_col_name not in cols_after_topo
        ):  # Check if 'VALUE1' exists but wasn't the new one
            print(
                f"    - Warning: Fallback rename. Renaming existing 'VALUE1' to '{topo_col_name}'."
            )
            points_gdf_after_topo.rename(
                columns={"VALUE1": topo_col_name}, inplace=True
            )
            ...
            print(
                f"    - Warning: Could not identify or rename column for Topo DEM. Found new: {new_cols_topo}. All: {list(cols_after_topo)}"
            )
            topo_col_name = None  # Flag that renaming failed

        # Save potentially renamed file before next extraction
        if topo_col_name:  # Only save if rename was successful or attempted
            points_gdf_after_topo.to_file(
                str(points_extracted_path), driver="ESRI Shapefile"
            )
        cols_before_toporaster_all_extraction = set(
            points_gdf_after_topo.columns
        )  # Update baseline columns
        ...
        if dem_toporaster_all_path.exists():
            ...
            print(
                f"  - Extracting values from TopoRaster_all DEM ({dem_toporaster_all_path.name})..."
            )
            wbt.extract_raster_values_at_points(
            ...
            new_cols_toporaster_all = list(
                cols_after_toporaster_all - cols_before_toporaster_all_extraction
            )
            ...
            if len(new_cols_toporaster_all) == 1:
                ...
                points_gdf_after_toporaster_all.rename(
                    columns={added_col_toporaster_all: toporaster_all_col_name},
                    inplace=True,
                )
                print(
                    f"    - Renamed third extracted value column '{added_col_toporaster_all}' to '{toporaster_all_col_name}'"
                )
            elif (
                "VALUE1" in new_cols_toporaster_all
            ):  # Check if 'VALUE1' is the *new* column this time
                print(
                    f"    - Warning: Could not uniquely identify new column. Attempting to rename 'VALUE1' to '{toporaster_all_col_name}'."
                )
                points_gdf_after_toporaster_all.rename(
                    columns={"VALUE1": toporaster_all_col_name}, inplace=True
                )
            elif (
                "VALUE1" in cols_after_toporaster_all
                and toporaster_all_col_name not in cols_after_toporaster_all
            ):  # Check if 'VALUE1' exists but wasn't the new one
                print(
                    f"    - Warning: Fallback rename. Renaming existing 'VALUE1' to '{toporaster_all_col_name}'."
                )
                points_gdf_after_toporaster_all.rename(
                    columns={"VALUE1": toporaster_all_col_name}, inplace=True
                )
                ...
                print(
                    f"    - Warning: Could not identify or rename column for TopoRaster_all DEM. Found new: {new_cols_toporaster_all}. All: {list(cols_after_toporaster_all)}"
                )
        ...
        if interp_col_name:
            required_cols.append(interp_col_name)
        if topo_col_name:
            required_cols.append(topo_col_name)
        if toporaster_all_col_name:
            required_cols.append(toporaster_all_col_name)

        # Check if *at least* the elevation field and one DEM column exist
        if len(required_cols) < 2 or not all(
            col in points_extracted_gdf.columns for col in required_cols
        ):
            ...
            if point_elev_field not in present_cols:
                raise ValueError(
                    f"Missing the original elevation point field '{point_elev_field}'. Cannot calculate RMSE."
                )
            elif len(required_cols) < 2:
                print(
                    f"    - Warning: No DEM columns were successfully extracted or renamed. Cannot calculate RMSE."
                )
                ...
                print(
                    f"    - Warning: Missing some DEM columns for RMSE: {missing}. Available: {present_cols}. Proceeding with available columns."
                )
        ...
        if len(required_cols) >= 2:
            ...
            points_valid_gdf = points_extracted_gdf.dropna(
        ...
        if n_points == 0:
            print(
                "    - Error: No valid points found after dropping NaN/Inf values or no DEM columns available. Cannot calculate RMSE."
            )
            ...
            if interp_col_name and interp_col_name in points_valid_gdf.columns:
                ...
                print(
                    f"    - RMSE (Interpolated DEM vs Points): {rmse_interp:.3f} (using {n_points} points)"
                )
            elif interp_col_name:
                print(
                    "    - Skipping RMSE calculation for Interpolated DEM due to column missing after NaN drop."
                )
            else:
                print(
                    "    - Skipping RMSE calculation for Interpolated DEM due to earlier column renaming issue."
                )
            ...
            if topo_col_name and topo_col_name in points_valid_gdf.columns:
                ...
                print(
                    f"    - RMSE (Topo DEM vs Points): {rmse_topo:.3f} (using {n_points} points)"
                )
            elif topo_col_name:
                print(
                    "    - Skipping RMSE calculation for Topo DEM due to column missing after NaN drop."
                )
            else:
                print(
                    "    - Skipping RMSE calculation for Topo DEM due to earlier column renaming issue."
                )

            if (
                toporaster_all_col_name
                and toporaster_all_col_name in points_valid_gdf.columns
            ):
                toporaster_all_dem_values = points_valid_gdf[toporaster_all_col_name]
                rmse_toporaster_all = sqrt(
                    mean_squared_error(measured, toporaster_all_dem_values)
                )
                print(
                    f"    - RMSE (TopoRaster_all DEM vs Points): {rmse_toporaster_all:.3f} (using {n_points} points)"
                )
            elif toporaster_all_col_name:
                print(
                    "    - Skipping RMSE calculation for TopoRaster_all DEM due to column missing after NaN drop."
                )
            else:
                print(
                    "    - Skipping RMSE calculation for TopoRaster_all DEM due to earlier column renaming issue or file not found."
                )
            ...
            if (
                ...
                rmse_results["DEM_Type"].append(
                    "TopoToRaster (ArcGIS Pro)"
                )  # Kept name
