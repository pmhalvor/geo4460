...
class BaseDemGenerator(ABC):
    ...
    def output_path_key(self) -> str:
        """Returns the key used in settings.output_files for this method's output DEM."""
        pass

    def get_output_path(self) -> Path:
        """Gets the full output path for the DEM generated by this method."""
        return self.output_files.get_full_path(self.output_path_key, self.output_dir)
...
class InterpolationDemGenerator(BaseDemGenerator):
    ...
    @property
    def output_path_key(self) -> str:
        return "dem_interpolated_tif"

    def generate(self, inputs: Dict[str, Path]) -> bool:
        self._log(f"Method: {self.method_name}...", indent=1)
        contour_points_path = inputs.get("contour_points_with_value")
        output_path = self.get_output_path()

        if not contour_points_path or not contour_points_path.exists():
            self._log(
                f"Required input 'contour_points_with_value' not found at {contour_points_path}",
                level="error",
                indent=2,
            )
            return False
        ...
        try:
            ...
            self.wbt.natural_neighbour_interpolation(
                i=str(contour_points_path),
                field="VALUE",  # Default field name from raster_to_vector_points
                output=str(output_path),
                cell_size=self.processing.output_cell_size,
            )
            self._log(f"Interpolated DEM saved: {output_path}", indent=2)
            if not output_path.exists():
                raise FileNotFoundError("Interpolated DEM file not created.")
            return True
...
class TinGriddingDemGenerator(BaseDemGenerator):
    ...
    def generate(self, inputs: Dict[str, Path]) -> bool:
        ...
        try:
            ...
            self._log(f"TIN Gridding DEM saved: {output_path}", indent=2)
            if not output_path.exists():
                raise FileNotFoundError("TIN Gridding DEM file not created.")
            return True
        except Exception as e:
            self._log(f"{self.method_name} failed: {e}", level="error", indent=1)
            return False
...
class StreamBurnDemGenerator(BaseDemGenerator):
    ...
    def generate(self, inputs: Dict[str, Path]) -> bool:
        self._log(f"Method: {self.method_name}...", indent=1)
        input_dem_path = inputs.get("tin_gridding_dem")  # Depends on TIN DEM
        output_path = self.get_output_path()
        ...
        if not self.stream_extract_threshold:
            ...
            return False
        if not input_dem_path or not input_dem_path.exists():
            self._log(
                f"Required input DEM for stream burning not found at {input_dem_path}",
                level="error",
                indent=2,
            )
            return False
        ...
        try:
            ...
            grass_loc_path = self._create_grass_location(
                temp_grass_db_path, location_name, input_dem_path
            )
...
class DemGenerationWorkflow:
    ...
    def __init__(
        ...
        river_shp_path: Optional[
            Path
        ] = None,  # Passed for context if needed by future methods
        ...
        # Intermediate paths managed by the orchestrator
        self.contour_raster_path = self.output_files.get_full_path(
            "contour_raster_temp", self.output_dir
        )
        self.contour_points_path = self.output_files.get_full_path(
            "contour_points_with_value_shp", self.output_dir
        )

        # Instantiate generator classes
        self.generators = {
            "interpolation": InterpolationDemGenerator(
                settings, wbt, self.output_dir, self.output_files, self.processing
            ),
            "tin_gridding": TinGriddingDemGenerator(
                settings, wbt, self.output_dir, self.output_files, self.processing
            ),
            "stream_burn": StreamBurnDemGenerator(
                settings,
                wbt,
                self.output_dir,
                self.output_files,
    ...
    def _prepare_contour_points(self) -> bool:
        ...
        self._log("Preparing shared contour points with elevation...", indent=1)
        try:
            ...
            self._log(
                f"Converting contour raster to points ({self.contour_points_path.name})...",
                indent=2,
            )
            self.wbt.raster_to_vector_points(
                i=str(self.contour_raster_path), output=str(self.contour_points_path)
            )
            if not self.contour_points_path.exists():
                raise FileNotFoundError("Points file with values not created.")
            self._log(
                f"Points with elevation saved: {self.contour_points_path}", indent=2
            )

            self._log("Contour points preparation successful.", indent=1)
            return True
        except Exception as e:
            self._log(f"Failed to prepare contour points: {e}", level="error", indent=1)
            return False
    ...
    def run_all(self) -> bool:
        ...
        interp_gen = self.generators["interpolation"]
        interp_inputs = {"contour_points_with_value": self.contour_points_path}
        results["interpolation"] = interp_gen.generate(interp_inputs)
        if not results["interpolation"]:
            overall_success = False

        # Step 3: Run TIN Gridding
        tin_gen = self.generators["tin_gridding"]
        tin_inputs = {"contour_points_with_value": self.contour_points_path}
        results["tin_gridding"] = tin_gen.generate(tin_inputs)
        if not results["tin_gridding"]:
            overall_success = False

        # Step 4: Run Stream Burning (if enabled and TIN succeeded)
        burn_gen = self.generators["stream_burn"]
        if self.processing.enable_stream_burning:
            if results["tin_gridding"]:
                burn_inputs = {"tin_gridding_dem": tin_gen.get_output_path()}
                results["stream_burn"] = burn_gen.generate(burn_inputs)
                if not results["stream_burn"]:
                    overall_success = False
            else:
                self._log(
                    f"Skipping {burn_gen.method_name} because prerequisite TIN Gridding failed.",
                    level="warning",
                    indent=1,
                )
            ...
            self._log(
                f"Skipping {burn_gen.method_name} (disabled in settings).", indent=1
            )
        ...
        for method, success in results.items():
            status = (
                ...
                if success
                    ...
                    "Failed"
                    if method != "stream_burn" or self.processing.enable_stream_burning
                    else "Skipped"
                )
            )
