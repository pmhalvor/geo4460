...
def generate_dems(
    ...
    stream_extract_threshold: Optional[
        int
    ] = None,  # Add threshold for r.stream.extract
):
    """
    ...
    contour_points_with_value_path = output_files.get_full_path(
        "contour_points_with_value_shp", output_dir
    )
    ...
    try:
        ...
        print(
            f"    - Converting contour lines to raster ({output_files.contour_raster_temp})..."
        )
        ...
        if not contour_raster_path.exists():
            raise FileNotFoundError(
                f"Intermediate contour raster not created: {contour_raster_path}"
            )

        # Convert contour raster back to points
        print(
            f"    - Converting contour raster to points ({output_files.contour_points_with_value_shp})..."
        )
        wbt.raster_to_vector_points(
            i=str(contour_raster_path), output=str(contour_points_with_value_path)
        )
    ...
    if settings.processing.enable_stream_burning:
        ...
        if not stream_extract_threshold:
            raise ValueError(
                "Stream burning enabled, but 'stream_extract_threshold' not provided in settings."
            )

        if (
            not settings.paths.grass_executable_path
            or not Path(settings.paths.grass_executable_path).exists()
        ):
            raise RuntimeError(
                f"GRASS executable not found at '{settings.paths.grass_executable_path}'. Please check config."
            )

        if not dem_topo_path.exists():
            raise FileNotFoundError(
                f"Input DEM for stream burning not found: {dem_topo_path}"
            )

        dem_burned_path = output_files.get_full_path(
            "dem_stream_burned_tif", output_dir
        )
        burn_value = (
            settings.processing.stream_burn_value
        )  # Get burn value from settings
        ...
        gisbase = str(
            grass_executable_path_obj.parent.parent
        )  # e.g., /Applications/GRASS-8.4.app/Contents
        ...
        if not grass_executable or not Path(grass_executable).exists():
            raise RuntimeError(
                f"GRASS executable not found via GRASSBIN env var: {grass_executable}"
            )
        ...
        try:
            ...
            raise RuntimeError(
                f"An unexpected error occurred during manual GRASS location creation: {e}"
            ) from e
        # --- End Manual Creation ---
        ...
        try:
            ...
            with Session(
                gisdb=str(temp_grass_db_path),
                location=location_name,
                mapset=mapset_name,
            ):  # No create_opts needed
                ...
                gs.run_command(
                    "g.region", raster=imported_raster_name, flags="p", verbose=True
                )

                # 1. Extract streams using r.stream.extract
                print(
                    f"      - Extracting streams using r.stream.extract (threshold={stream_extract_threshold})..."
                )
                gs.run_command(
                    "r.stream.extract",
                ...
                print(
                    f"      - Burning extracted streams into DEM using r.mapcalc (burn value={burn_value})..."
                )
                ...
                try:
                    ...
                    if not map_info or not map_info.get("name"):
                        raise RuntimeError(
                            "Map 'dem_burned' not found by find_file after r.mapcalc."
                        )
                ...
                print(
                    f"      - Exporting burned DEM as Float32 to: {dem_burned_path}..."
                )
                gs.run_command(
                    "r.out.gdal",
                    input="dem_burned",
                ...
                print(
                    f"    - Stream-extracted and burned DEM saved to: {dem_burned_path}"
                )
            ...
            raise RuntimeError(
                "GRASS stream extraction/burning failed within session."
            ) from e
        finally:
        ...
        if not dem_burned_path.exists():
            raise FileNotFoundError(
                f"Stream-burned DEM file was not created by GRASS session: {dem_burned_path}"
            )
        else:
    ...
    if (
        "temp_grass_db_path" in locals() and temp_grass_db_path.exists()
    ):  # Check if var exists before cleanup
        print(
            f"    - Cleaning up temporary GRASS database directory: {temp_grass_db_path}"
        )
        ...
        try:
            ...
            print(
                f"Warning: Could not remove temporary GRASS directory {temp_grass_db_path}: {e}"
            )
...
if __name__ == "__main__":
    ...
    if not contour_shp_test_path.exists():
        ...
        print(
            "Please run the 'load_data' step first (e.g., via the main workflow) to generate intermediate files."
        )
        exit()
    if settings.processing.enable_stream_burning and not river_shp_test_path.exists():
        print(
            f"ERROR: River shapefile for stream burning test not found at {river_shp_test_path}"
        )
        print(
            "Please run the 'load_data' step first (e.g., via the main workflow) to generate intermediate files."
        )
    ...
    if settings.processing.enable_stream_burning:
        ...
        print(
            f"Stream Burning Enabled: True (Value: {settings.processing.stream_burn_value})"
        )
