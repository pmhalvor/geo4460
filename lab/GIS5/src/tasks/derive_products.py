...
def generate_derived_products(
    settings: BaseModel, wbt: WhiteboxTools, dem_interp_path: Path, dem_topo_path: Path
):
    """
    ...
    # Define output paths
    contours_interp_path = output_files.get_full_path(
        "contours_interpolated_shp", output_dir
    )
    contours_topo_path = output_files.get_full_path("contours_topo_shp", output_dir)
    hillshade_interp_path = output_files.get_full_path(
        "hillshade_interpolated_tif", output_dir
    )
    ...
    if not dem_interp_path.exists():
        print(
            f"Error: Interpolated DEM not found ({dem_interp_path}). Skipping derived product generation."
        )
        return  # Or raise error
    if not dem_topo_path.exists():
        print(
            f"Error: Topo DEM not found ({dem_topo_path}). Skipping derived product generation."
        )
        return
    ...
    try:
        ...
        wbt.hillshade(dem=str(dem_topo_path), output=str(hillshade_topo_path))
        ...
        wbt.slope(dem=str(dem_topo_path), output=str(slope_topo_path))
