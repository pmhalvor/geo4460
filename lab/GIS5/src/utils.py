...
def get_common_extent_and_crs(layers: List[gpd.GeoDataFrame]) -> Tuple[float, float, float, float, Optional[CRS]]:
    """
    ...
    if common_crs is None:
        print("Warning: First input layer CRS is None. Cannot ensure CRS consistency or reproject.")
    ...
    for i, layer in enumerate(layers[1:], start=1):
        ...
        if common_crs is not None and layer_crs != common_crs:
            print(f"Warning: CRS mismatch found in layer {i+1}. Expected {common_crs}, got {layer_crs}. Reprojecting...")
            try:
                layer = layer.to_crs(common_crs)
                print(f"  - Layer {i+1} reprojected successfully.")
            except Exception as e:
                raise ValueError(f"Failed to reproject layer {i+1} to {common_crs}: {e}")
        elif common_crs is None and layer_crs is not None:
             print(f"Warning: First layer had no CRS, but layer {i+1} has CRS {layer_crs}. Cannot guarantee consistency.")
...
def find_elevation_field(gdf: gpd.GeoDataFrame, field_candidates: List[str]) -> Optional[str]:
    """
    ...
    for field in field_candidates:
        if field in gdf.columns:
            print(f"Elevation field found: '{field}'. Sample values: {gdf[field].head().tolist()}")
            return field

    print(f"Warning: Could not automatically identify elevation field from candidates: {field_candidates}.")
