...
if not gisbase:
    ...
    potential_gisbase = "/Applications/GRASS-8.4.app/Contents/Resources" # Adjust version if needed
    if Path(potential_gisbase).exists():
        ...
        print("Please set GISBASE to your GRASS installation directory (e.g., /Applications/GRASS-8.4.app/Contents/Resources)")
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        if not grass_executable or not Path(grass_executable).exists():
             ...
             grass_executable = "/Applications/GRASS-8.4.app/Contents/MacOS/Grass.sh" # Default guess
             if not Path(grass_executable).exists():
                 print(f"ERROR: GRASS executable not found via GRASSBIN or default path ({grass_executable}).")
                 sys.exit(1)
             else:
                 print(f"Warning: GRASSBIN env var not found, using default: {grass_executable}")
        ...
        with Session(gisdb=str(gisdb), location=location_name, mapset=mapset_name): # No create_opts needed now
            ...
            try:
                ...
                map_info = gs.find_file(name='dem_burned', element='cell') # Check for raster cell data
                ...
                if not map_info or not map_info.get('name'):
                     raise RuntimeError("ERROR: dem_burned map not found by find_file after r.mapcalc (empty info)")
        ...
        if not dem_burned_path.exists():
            ...
            raise FileNotFoundError(f"Stream-burned DEM file was NOT created despite apparent success: {dem_burned_path}")
        else:
...
if __name__ == "__main__":
    ...
    os.environ['DYLD_LIBRARY_PATH'] = f"{gisbase_lib}:{os.environ.get('DYLD_LIBRARY_PATH', '')}"
