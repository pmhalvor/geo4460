...
class CostDistance(FeatureBase):
    ...
    def load_data(self):
        ...
        logger.info("Validating CostDistance input paths...")
    ...
    def _build_cost_raster(self):
        ...
        final_output_key = "normalized_cost_distance"
        ...
        try:
            ...
            return str(final_output_path) # Return path as string for dask
        except Exception as e:
...
if __name__ == "__main__":
    ...
    logger.info("--- Running cost_distance.py Standalone Test ---")
    ...
    if settings:
        ...
        # --- Test CostDistance Feature ---
        if inputs_ok:
            try:
                logger.info("--- Testing CostDistance Feature ---")
                cost_feature = CostDistance(
                    settings=settings,
                    wbt=wbt,
                ...
                if delayed_task is not None:
                    ...
                    # The task itself returns the path string or None.
                    computed_result = dask.compute(delayed_task)[0]
                    logger.info("Build computation completed.")

                    # Process the computed result
                    raster_paths = [computed_result] if computed_result and Path(computed_result).exists() else []

                    if not raster_paths:
                         logger.warning("Cost raster generation appears to have failed or file not found after compute.")

                    # --- Display Raster on Folium Map ---
                    if raster_paths:
                        logger.info("Generated Normalized Cost Raster:")
                        for path_str in raster_paths: # Loop will run once if successful
                            ...
                            try:
                                ...
                                cmap_name='plasma', # Use a different cmap for cost
                                opacity=0.8,
