...
class CostDistance(FeatureBase):
    ...
    def build(self):
        """Builds the normalized cost raster."""
        # load_data called in __init__
        task = self._build_cost_raster()
        # Compute the single task. result[0] because compute returns a tuple
        result = dask.compute(task)[0]
        # Result is the path (string) or None
        successful_outputs = [result] if result else []
    ...
    if settings:
        ...
        if inputs_ok:
            try:
                ...
                raster_paths = cost_feature.build() # Returns list of paths
                logger.info("CostDistance build process completed.")
