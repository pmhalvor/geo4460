...
class Traffic(FeatureBase):
    ...
    def build(self):
        ...
        # 5. Compute the tasks
        logger.info(f"Starting computation for {len(tasks)} traffic density rasters...")
        results = dask.compute(*tasks) # Unpack tasks for compute
        logger.info("Dask computation finished.")

        # 6. Collect successful results
        successful_outputs = [r for r in results if r is not None]
        logger.info(f"Successfully generated {len(successful_outputs)} traffic density rasters.")
...
if __name__ == "__main__":
    ...
    if settings:
        ...
        try:
            ...
            raster_paths = traffic_feature.build()
            logger.info("Traffic build process completed.")

            # 3. Display Rasters on Folium Map
            if raster_paths:
