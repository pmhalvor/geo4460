...
def get_stations_gdf():
    ...
    for road_id in ROAD_CATEGORY_IDS:
        with open(STATIONS_PATH_TEMPLATE.format(id=road_id)) as f:
            ...
            assert list(stations["data"]["trafficRegistrationPoints"][0].keys()) == [
                "id",
                "name",
                "location",
            ]
...
def get_traffic_df(interval: str = "day"):
    ...
    df = pd.read_csv(
        datapath, encoding="ISO-8859-1", header=0, sep=";", dtype="unicode"
    )
...
def get_oslo_traffic(interval: str = "day"):
    ...
    return traffic_df.merge(
        stations_df, left_on="Trafikkregistreringspunkt", right_on="id", how="left"
    )
